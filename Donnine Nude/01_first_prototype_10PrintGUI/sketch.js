...
function draw() {
  ...
  function star(x, y, radius1, radius2, npoints) {
    ...
    for (let a = 0; a < TWO_PI; a += angle) {
      let sx = x + cos(a) * radius2 * noise(0.01 + 500); //per renderli dinamici aggiungi"* frameCount" dopo 0.01
      let sy = y + sin(a) * radius2 * noise(0.01 + 500); //per renderli dinamici aggiungi"* frameCount" dopo 0.01
      vertex(sx, sy);
